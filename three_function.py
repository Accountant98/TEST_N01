...
def query_data(project_name, market, powertrain, develop_case, group, lot):
    ...
    engine = create_engine("mysql+mysqlconnector://root:Sql123456@localhost/db_21xe")
    ...
    if project is not None:
        ...
        header_query = session.query(Header).all()
        ...
        if header_df is not None:
            st.session_state.edited_data = result_df_matched

            st.write("Query Result:")

            with st.form("edit_form"):
                data_edit = st.data_editor(st.session_state.edited_data)

                submitted_data = st.form_submit_button("Save Changes")
                return data_edit, submitted_data, session, result_df_matched, id_project, id_app_list
    else:
        st.error("Project not found in the database.")

    session.close()

def update_edit(data_edit, submitted_data, session, result_df_matched, id_project, id_app_list):
    if submitted_data:
        try:
...
def update_new(project_name, market, power_train, develop_case, df):
    engine = create_engine("mysql+mysqlconnector://root:Sql123456@localhost/test_alchemy")
    Base.metadata.create_all(engine)
    Session = sessionmaker(bind=engine)
    session = Session()
...
# data_edit, submitted_data, session, result_df_matched, id_project, id_app_list = query_data("WZ1J", "EUR", "e-Power", "CASE2", "", "ALL")
