...
def create_cadics(case, market, powertrain, car):
    ...
    write_excel(my_dict_data, link_cadic, frame_header)
    return "Completed!!!"
...
def get_infor_car(df_spec):
    dict_grade={}
    dict_optioncode={}
    frame_header = pd.read_csv("form_out/form_cadics.csv", header=None)
...
"""
def write_excel(my_dic_data, file_name, frame_header):
    ...
    data_new.to_csv(file_name, index=False, header=None)
...
def get_group_karenhyo12(folder_data,car,folder_out):
    ...
    return link_file_spec,dic_group_karenhyo12
