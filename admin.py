    ...
    with col_left:
        with st.form('input_form'):
            ...
            if col_left_spec_grid.form_submit_button("Create Cadics", use_container_width=True)==True and get_data("running")==0 :
                set_data("running",1) 
                notice=create_cadics(st.session_state.case,st.session_state.plant,st.session_state.pwt,st.session_state.code)
                st.write(notice)
                list_file,folder_output,name_zip=check_file_out(st.session_state.code,st.session_state.pwt,st.session_state.plant,st.session_state.case)
                set_state(list_file,folder_output,name_zip)
                set_data("running",0)
...
def set_state(list_file,folder_output,name_zip):
    ...
    list_link=["CADICS","Car配車要望表","WTC仕様用途一覧表","WTC要望集約兼チェックリスト","実験部品","特性管理部品リスト","File Log"]
