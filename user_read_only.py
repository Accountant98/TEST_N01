    ...
    with col_left:
        with st.form('input_form'):
            ...
            if col_left_spec_grid.form_submit_button("View Data", use_container_width=True):
                ...
                list_link=["CADICS","Car配車要望表","WTC仕様用途一覧表","WTC要望集約兼チェックリスト","実験部品","特性管理部品リスト","File Log.xlsx"]
...
def view(user):
    ...
    if get_data("flag_view") == 1:
        if selected_option_output_select == "CADICS 項目" :
            ...
            data,flag= read_cadics(get_data("CADICS"))
            if user=="staff":
                button_select_caout_grid.dataframe(data,height=525)
            if user!="staff":
                if flag==1:
                    data_edit=button_select_caout_grid.data_editor(data,height=525)
                    with row_butt[3]:
                        if st.button("SAVE",use_container_width=True) and get_data("CADICS")!=None:
                            data_edit.to_csv(get_data("CADICS"),index=False,header=None)
                            st.write("Save Completed!!!")
                else:
                    button_select_caout_grid.dataframe(data,height=525)

            set_data("link", get_data("CADICS"))
            set_data("name", "CADICS_ALL.xlsx")   
