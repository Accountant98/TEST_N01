...
#===========================load_infomation_input==========================
    ...
    if link_spec==None or len(dict_group_karenhyo3)==0 or len(dict_group_karenhyo4)==0 or os.path.exists(file_cadic)==False:
        return "Check input again!!!"
#==========================Du lieu tong=====================================
    cadics_all=pd.read_csv(file_cadic,header=None,low_memory=False)
...
def filter_cadic(lot,group,df,car_number):
    ...
    head=df.iloc[0:6]
    try:
        data_=df.loc[(df[col_evaluate]=="yes") & (df[col_group]==group)& (df[130+car_number]!="要望仕様が存在しない")]
    except:
